...
export default function DashboardPage() {
  ...
  return (
    <main id="main-content" className="container main-content">
      <PageHeader title="Dashboard" description="Aggregate view of the demo market ecosystem." />
      ...
      <div className="overview-grid dashboard-grid">
        ...
        <div className="card">
          ...
          <div className="dashboard-body">
            {distribution === null ? <TableSkeleton rows={5} /> : <DistributionChart buckets={distribution} />}
          </div>
        </div>
      </div>
              ...
              {feed.map((item) => (
                <li key={item.id}>
                  <button type="button" className="feed-row" onClick={() => open(item.account)}>
                    <Avatar account={item.account} size={26} radius={8} />
      ...
      <section className="section" aria-labelledby="categories-heading">
        ...
        <div className="card">
          {categories === null ? <TableSkeleton rows={6} /> : <CategoryBreakdown categories={categories} />}
        </div>
      </section>
    </main>
  );
